...
#            self.data.update({"customer":  self.ocapi("customer", "customers")})
    ...
    def get_product_specs(self, specurl, is_child):
        ...
        if "service_category_limit" in product.get("characteristics"):
            specs.update({'included_volume': product.get("characteristics").get("service_category_limit")})
    ...
    def get_product_price(self, product):
        ...
        if "characteristics" in product and "salespricevatincl" in product.get('characteristics'):
        ...
        if response:
            for internetusage in response.get('internetusage'):
                ...
                if "included_volume" in specs:
                    ...
                    includedvolume = kb_to_gb(usage.get('includedvolume') + usage.get('extendedvolume').get("volume"))
                totalusage = usage.get('totalusage')

                total_usage_with_offpeak = kb_to_gb(totalusage.get('offpeak')+totalusage.get('wifree'))
                ...
                self.data.get('products').update({
                    businessidentifier: {
                        ...
                        'total_usage_with_offpeak': kb_to_gb(totalusage.get('peak')+totalusage.get('offpeak')),
    ...
    def get_legacy_tv(self, product):
        """Retrieve internet details."""
        data = {}
        mapping = ['identifier','customerproductid','accountnumber', 'label', 'rateclassdescription']
        for key in mapping:
            data.update({
                key: product.get(key)
            })
            ...
            for digitaltvunbilledusage in response.get('digitaltvunbilledusage'):
                for key in digitaltvunbilledusage:
                    if "usage" in key and "total" in digitaltvunbilledusage.get(key):
                        open_bill += str_to_float(digitaltvunbilledusage.get(key).get('total'))
            self.data.get('bills').update({
                'dtv': {'total': open_bill, 'unit': 'EURO', 'data': digitaltvunbilledusage}
            })
    ...
    def get_product(self, product, is_child=False):
        ...
        if 'productType' in product and product.get('productType') == "bundle" and len(product.get('children')):
    ...
    def get_netcracker(self):
        ...
        products = self.ocapi("product", "products", params={"status":"ACTIVE,ACTIVATION_IN_PROGRESS"})
